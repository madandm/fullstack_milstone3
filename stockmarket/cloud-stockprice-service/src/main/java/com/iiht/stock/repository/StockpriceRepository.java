...
public interface StockpriceRepository extends JpaRepository<StockpriceEntity, Integer>{
	@Query(value = "select s.COMPANY_CODE, s.COMPANY_NAME, s.CURRENT_PRICE, s.OPEN_PRICE, s.HIGH_PRICE, s.LOW_PRICE, date_format(s.PRICE_DATE, '%Y-%m-%d') as PRICE_DATE, DAYOFWEEK(s.PRICE_DATE) as week from s_stock_price s where (s.COMPANY_CODE = :key or s.COMPANY_NAME = :key) and if(:from!=null or :from!='',s.PRICE_DATE between :from and :end,1=1)", nativeQuery = true)
	public List<StockpriceEntity> findStockPrice(@Param("key")String key, @Param("from")String from, @Param("end")String end);
	
	@Query(value = "select  date_format(MAX(s.PRICE_DATE), '%Y-%m-%d') as month from s_stock_price s where s.COMPANY_CODE = :key or s.COMPANY_NAME = :key", nativeQuery = true)
